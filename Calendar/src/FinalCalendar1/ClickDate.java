	...
	public void actionPerformed(ActionEvent e) {
		...
		for (int i = 1; i < CalendarMemo.date.length; i++) {
			if (c.getText().equals("" + (i - firstDay + 1))) {
				...
				CalendarMemo.dateSchedule[i].setBackground(Color.GREEN);
				CalendarMemo.dateSchedule[i].setBackground(Color.GREEN);

			} else{
				CalendarMemo.datePanel[i].setBackground(Color.WHITE);
				CalendarMemo.date[i].setBackground(Color.WHITE);
				if(CalendarMemo.dateSchedule[i].getText().length() != 0)
					CalendarMemo.dateSchedule[i].setBackground(new Color(217,217,217));
				else
					CalendarMemo.dateSchedule[i].setBackground(Color.WHITE);
